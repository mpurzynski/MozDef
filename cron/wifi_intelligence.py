...
class UsernameNetResolve:
    def __init__(self):
        ...
        # self.es.create_index("intelligence")
    ...
    def find_mac_by_ip(self):
        ...
        search_query.add_must(
            [TermMatch("category", "syslog"), TermMatch("processname", "dhcpd")]
        )
        search_query.add_should(
            [
                QueryStringMatch("summary: DHCPOFFER"),
                QueryStringMatch("summary: DHCPACK"),
            ]
        )
        ...
        for event in events["hits"]:
            ...
            if match_ip + match_mac in self.seenRing.get():
                logger.debug("Cache HIT on {0} <- {1}".format(match_ip, match_mac))
                continue
            else:
                # We also cache negative lookups.
                # If the Radius log is late - the next script invocation will catch it
                # because there is a 1 minute of sliding window and the cache
    ...
    def find_username_by_mac(self, match_mac, match_ip):
        ...
        logger.debug("Looking up {0} in Radius logs".format(match_mac))
        search_query = SearchQuery(minutes=3)
        search_query.add_must(
            [
